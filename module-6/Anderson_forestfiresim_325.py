...
FIRE = '@'
EMPTY = ' '
WATER = '~'  # New water character for lake

# (!) Try changing these settings to anything between 0.0 and 1.0:
INITIAL_TREE_DENSITY = 0.20  # Amount of forest that starts with trees.
GROW_CHANCE = 0.5  # Chance a blank space turns into a tree.
...
def createNewForest():
    ...
    # Add lake roughly in the center
    lake_width = 15
    lake_height = 6
    ...
    for x in range(lake_x_start, lake_x_start + lake_width):
        for y in range(lake_y_start, lake_y_start + lake_height):
            forest[(x, y)] = WATER  # Set these cells as water
